...
public class AddressBookMain {
	...
	public static int writeJson(ArrayList<Address> addresBook) throws IOException {
		...
		List list = addresBook.stream().collect(Collectors.toList());
		String json = gson.toJson(list);
		FileWriter writer = new FileWriter("jsonOfAddressBook.json");
		writer.write(json);
		writer.close();
		return list.size();
	}
	...
	public static int readJson() {
		int count = 0;
		try {
			Reader reader = Files.newBufferedReader(Paths.get("jsonOfAddressBook.json"));
